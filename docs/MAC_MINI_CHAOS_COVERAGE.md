...
| C-DISK-08 dm-flakey / dm-error / drop_writes | `dm-flakey`, `dm-drop-writes-after-ack-*`, release-gate `dm-error` | dm-flakey and drop-writes are executable with a pre-provisioned device and exactly one RustFS pod on the DM node. The release gate runs one DM scenario per invocation (`RELEASE_GATE_DM_SCENARIO`, default `dm-flakey`) against `RUSTFS_RELEASE_GATE_DM_STORAGE_CLASS`. A single-node or unspread multi-pod tenant is `SKIP-dm-topology` (not a pass) and, when that skip is decided before `dm-run`, the existing Tenant is left in place. After a `dm-run` that starts, the gate deletes a Tenant that uses the DM class and returns Released 100Gi PVs to Available (`lost+found` stays). Emptying one PV can fail without skipping the rest; that PV keeps `claimRef` and the command exits non-zero. `dm-error` flushes the filesystem before the error target, direct-reads a marker while that target is active, and restores with `dmsetup suspend --nolockfs`. `recovered` also requires a clean superblock and a new O_DIRECT read. A Bound PV, another mount, an extra open holder, or the same major:minor in another mount namespace (including hostPath) is `SKIP-dm-in-use` (not a pass) and the lab path stays mounted. Systemd slave copies of the host mount (`master:N` equals the host `shared:N`) are not holders; a `kubepods` cgroup still is. | manual (`SKIP-no-dm` otherwise, `SKIP-dm-not-selected` for the DM scenarios that this run does not execute, `SKIP-dm-topology` when the DM node cannot host exactly one pod, `SKIP-dm-in-use` when the target volume is held) |
...
| C-TOPO-05 cold metadata on a survivor | release-gate `quorum-edge-cold-read` and `quorum-edge-readiness` | Regression for 1.0.1-preview.11. `cold_bucket` is a bool: this survivor never served the bucket. The cold-read case requires GET success, rejected writes, and `/health/live` 200. `/health/ready` is a separate case, because readiness 503 on write-quorum loss is a different bug. | CI amd64, Mac Mini arm64 |
