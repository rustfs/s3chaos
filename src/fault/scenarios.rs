...
pub const FAULT_SCENARIO_CATALOG: &[FaultScenarioSpec] = &[
    ...
    FaultScenarioSpec {
        ...
        description: "SIGKILL the highest-ordinal RustFS Pod every 15 seconds while the workload runs, and verify peers keep serving committed objects. Chaos Mesh Schedule is the actuator. If status.time is still empty after 30 seconds, the harness deletes nothing and drives the same pinned pod-kill from a controller loop, because Chaos Mesh 2.8.3 @every schedules can fail to fire.",
        ...
        target_proof: &[
            ...
            "activation waits until that Pod's UID disappears at least once; Schedule status.time is the preferred proof, and a controller loop is the fallback when time is still empty after 30 seconds",
            "the Schedule stays armed when it fired; otherwise the controller loop keeps killing that Pod for the workload and later kills are still the fault",
        ],
        validation: "at least one kill of the selected Pod is observed before the workload, either the Schedule stays armed or the controller loop is still driving pinned pod-kills while the mixed workload runs, every committed object remains readable and the workload meets the availability floor, and after the actuator is removed the Tenant returns Ready with committed objects intact",
